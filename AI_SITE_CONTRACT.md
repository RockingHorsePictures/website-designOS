...
4. Components consume semantic colour tokens from `src/design-system/tokens.ts` and font-family/weight tokens from `src/design-system/typography.ts`. Editors control approved body/heading fonts and body/heading/emphasis weights. Font sizes, spacing, breakpoints and motion remain code-controlled. New fonts require an approved, properly licensed self-hosted asset, a registry entry and a schema migration; preserve stored IDs. Current defaults are neutral fixtures, not branding decisions.
...
See DESIGN_HANDOFF.md for the practical workflow and starting prompt. Until the foundation PR is merged, start from foundation/design-os, not main.
